      ...
      <q-modal-layout>
        ...
        <div class="layout-padding group">
          ...
          <div
            ...
            :style="{'background-color': timetable.timetype_id.$model === null ? null : timetableTypes && timetableTypes[timetable.timetype_id.$model] ? timetableTypes[timetable.timetype_id.$model].color : null}"
            ...
            <div class="row q-px-sm" v-if="advancedTimetable">
              <q-select class="col-6" :options="options.timetype_id" v-model="timetable.timetype_id.$model"></q-select>
              ...
              <div class="col-6 q-pt-sm">
                <q-checkbox v-model="timetable.start_require_event.$model" label="debe marcar inicio"></q-checkbox>
              </div>
              <div class="col-6 q-pt-sm">
                <q-checkbox v-model="timetable.end_require_event.$model" label="debe marcar fin"></q-checkbox>
              </div>
            </div>
          </div>
              ...
              <q-popover self="top middle" anchor="bottom middle">
                <q-list link>
                  <q-item v-for="(preset, index) in timetablePresets" :key="index" v-close-overlay @click.native="$v.item.timetable.$model.push(clonePreset(preset))">
                    ...
                    <q-item-side>
                      <q-chip :style="{ 'background': timetableTypes && timetableTypes[preset.timetype_id] ? timetableTypes[preset.timetype_id].color : null}">{{timetableTypes && timetableTypes[preset.timetype_id] ? timetableTypes[preset.timetype_id].code : null}}</q-chip>
                    </q-item-side>
                  </q-item>
                </q-list>
              </q-popover>
